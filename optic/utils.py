...
.. autosummary::
   ...
   llr2bitProb            -- Convert log-likelihood ratios (LLRs) to bit probabilities.
...
def llr2bitProb(llr):
    """
    Converts log-likelihood ratios (LLRs) to bit probabilities.

    Parameters
    ----------
    llr : ndarray
        Log-likelihood ratios defined as :math:`\\log(P(b=1)/P(b=0))`.

    Returns
    -------
    ndarray
        Probabilities :math:`P(b=1)` for each input LLR.

    Notes
    -----
    - The LLRs generated by the functions of `optic.comm.metrics` follow the
      :math:`\\log(P(b=0)/P(b=1))` convention, hence they must be sign-flipped
      before being passed to this function.
    """
    return 1 / (1 + np.exp(-llr))
