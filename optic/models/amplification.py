...
.. autosummary::
   ...
   get_mode_radius      -- Determines the mode radius of the fiber.
   edfParams            -- Returns the EDFA parameters.
   edfaArgs             -- Returns the EDFA arguments.
   edfaSM               -- Returns the EDFA state matrix.
"""
...
def get_spectrum(
    x, Fs, Fc, xunits="m", yunits="dBm", window=mlab.window_none, sides="twosided"
):
    """
    ...
    """
    spectrum, frequency = mlab.magnitude_spectrum(x, Fs=Fs, window=window, sides=sides)
...
def gilesSpatial(z, P, properties, param_edf):
    ...
    xi_k = intOL * (properties.absCoef + properties.gainCoef) / properties.gamma - (
        properties.absCoef + properties.lossS
    )
    tauASE = (
        intOL
        * (properties.gainCoef / properties.gamma)
        * Planck
        * properties.freq
        * properties.noiseBand
    )
    return properties.uk * (P * xi_k + properties.ASE * tauASE)
...
def getN2Pop(P, properties):
    ...
    if properties.algo == "Giles_spectrum":
        ...
        n2_normT1 = (properties.tal / Planck) * (
            properties.i_k @ np.transpose(P * properties.absCross / properties.freq)
        )
        n2_normT2 = (properties.tal / Planck) * (
            properties.i_k
            @ np.transpose(
                P * (properties.absCross + properties.emiCross) / properties.freq
            )
        ) + 1
...
def edfaSM(Ei, Fs, Fc, param_edfa):
    ...
    param_edfa.absCoef = np.interp(
        c / param_edfa.freq, param_edf.lbFl, param_edf.absCoef
    )
    param_edfa.gainCoef = np.interp(
        c / param_edfa.freq, param_edf.lbFl, param_edf.gainCoef
    )
    # BCKPUMP + BASEX + BASEY
    ...
    if param_edfa.algo == "Giles_spatial":
        # SIGNALX + SIGNALY + FASEX + FASEY + FORPUMP
        param_edfa.absCross = np.interp(
            c / param_edfa.freq, param_edf.lbFl, param_edf.absCross
        )
        param_edfa.emiCross = np.interp(
            c / param_edfa.freq, param_edf.lbFl, param_edf.emiCross
        )
        param_edfa.gamma = np.interp(
            c / param_edfa.freq, param_edf.lbFl, param_edf.gamma
        )
    ...
    while (np.abs(errorAutoCrtl) > param_edfa.tolCtrl) and (tryCtrlLoop < MaxTry):
        ...
        if (param_edfa.type == "AGC") or (param_edfa.type == "APC"):
            ...
            pid = PID(
                ...
                param_edfa.ki,
                param_edfa.kd,
                setpoint=param_edfa.value,
                output_limits=(-pumpPmpFor / 2, pumpPmpFor / 2),
            )
