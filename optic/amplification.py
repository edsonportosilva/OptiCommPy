...
def edfaSM(Ei, Fs, Fc, param_edfa):
    ...
    param_edfa.value     = getattr(param_edfa, "value", 20) # dB (for gain) and dBm (for power)
    ...
    if (param_edfa.algo not in ("Giles_spatial", "Giles_spectrum", "Saleh", "Jopson", "Inhomogeneous")):
        raise TypeError('edfaSM.algo invalid argument - [Giles_spatial, Giles_spectrum, Saleh, Jopson, Inhomogeneous].')
    ...
    if (len(freqPmpFor) != len(pumpPmpFor)):
        raise TypeError('edfaSM.forPump invalid argument - number of signals (freq and pump) must be igual.')
    if (len(freqPmpBck) != len(pumpPmpBck)):
        raise TypeError('edfaSM.bckPump invalid argument - number of signals (freq and pump) must be igual.')
    ...
    param_edfa.ASE  = np.concatenate([np.zeros(isy*lenFqSg), np.ones(isy*lenASE), np.zeros(lenPmpFor)])
    ...
    if (param_edfa.algo == 'Giles_spatial'):        
        ...
        param_edfa.i_k      = interpolate.interp2d(lbFl, param_edfa.r, param_edf.i_k, kind='cubic')(c/param_edfa.freq, param_edfa.r)
        # BCKPUMP + BASEX + BASEY
    ...
    while ((np.abs(errorAutoCrtl) > param_edfa.tolCtrl) and (tryCtrlLoop < MaxTry)):
        ...
        while ((np.mean(np.abs(errorCvg)) > param_edfa.tol) and (tryLoop < MaxTry)):
            ...
            tryLoop = tryLoop + 1
            if tryLoop == MaxTry:
                logg.info('Convergence fail: number of loops greater than max (%d)' %(MaxTry))
        # Automatic gain or power control
        if (param_edfa.type == 'AGC') or (param_edfa.type == 'APC'):
            power_out = np.sum(Pout[np.concatenate([idxPS, idxPAF]),-1]) # Output power - Signal + For. ASE
            ...
            pid = PID(0.01, 0.01, 0.05, setpoint = param_edfa.value, output_limits = (0, 300e-3))
            ...
            if tryCtrlLoop == MaxTry:
                logg.info('Control fail: number of loops greater than max (%d)' %(MaxTry))
        else:
            errorAutoCrtl = 0
    ...
    Eout = np.reshape(np.sqrt(Pout[idxPS, -1], dtype = np.complex), (lenFqSg, isy), order='F')
...
def edfParams(param_edfa):
    ...
    if (param_edfa.gmtc == 'LP01'):
        ...
        if (param_edfa.algo == "Giles_spatial"):
            ...
            ik = lambda r: 1 / np.pi * (v / (param_edfa.a * V) * jv(0, u * param_edfa.r / param_edfa.a) / jv(1, u)) ** 2
...
def OSA(x, Fs, Fc):
    ...
    specX, freqs = mlab.magnitude_spectrum(x[:,0], Fs=Fs, window = mlab.window_none, sides='twosided')
    freqs += Fc
    ...
    minY = -70
    if (isy == 2):
        specY,freqs = mlab.magnitude_spectrum(x[:,1], Fs=Fs, window = mlab.window_none, sides='twosided')
